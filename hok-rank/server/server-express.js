...
app.post('/upload-hok', function (req, res) {
    jsonFile.writeFile('./public/cache2.json', req.body, function (err) {
        if (err) return res.send({ color: 'btn btn-danger', info: '列表保存失败' });
        res.send({ color: 'btn btn-info', info: '列表已保存' });
    });
})

app.get('/get-hok', function (req, res) {
    jsonFile.readFile('./public/cache2.json', function (err, jsonData) {
        if (err) return res.send({ color: 'btn btn-danger', info: '列表读取失败' });
        res.send(jsonData);
    });
})
