...
`
    ...
    export const Config: Schema<Config> = Schema.object({
        ...
        engine: Schema.union([
            Schema.const('colorfulcocktail_diffusion' as string).description('彩色鸡尾酒'),
            Schema.const('stable_diffusion' as string).description('通用'),
            Schema.const('acgn_diffusion' as string).description('动漫'),
            Schema.const('anything_diffusion' as string).description('新动漫'),
            Schema.const('redshift_novelai_sd_merge_diffusion' as string).description('3D建模风格'),
            Schema.const('mid_stable_diffusion' as string).description('艺术感强化'),
            Schema.const('paper_cut_engine' as string).description('剪纸艺术'),
        ]).description('绘画引擎' as string).default('colorfulcocktail_diffusion'),
        ratio: Schema.number().description('0是4 : 3, 1是3 : 4, 2是正方形, 3是16 : 9, 4是9 : 16。').default(3),
        style: Schema.union([
            Schema.const('新海诚' as string).description('新海诚'),
            Schema.const('经典动漫' as string).description('经典动漫'),
            Schema.const('油画' as string).description('油画'),
            Schema.const('水彩绘画' as string).description('水彩绘画'),
            Schema.const('中国画' as string).description('中国画'),
            Schema.const('蒸汽波' as string).description('蒸汽波'),
            Schema.const('莫奈' as string).description('莫奈'),
            Schema.const('二次元色彩大师' as string).description('二次元色彩大师'),
            Schema.const('真实感照片' as string).description('真实感照片'),
            Schema.const('剪纸艺术' as string).description('剪纸艺术'),
            Schema.const('赛博朋克' as string).description('赛博朋克'),
            Schema.const('保罗·塞尚' as string).description('保罗·塞尚'),
            Schema.const('托马斯·科尔' as string).description('托马斯·科尔'),
            Schema.const('村上隆' as string).description('村上隆'),
            Schema.const('穆夏' as string).description('穆夏'),
            Schema.const('毕加索' as string).description('毕加索')
        ]).description('style').default('新海诚' as string),
