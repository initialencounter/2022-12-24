...
if CHAT_LANG == 'English':
    args.MODEL_NAME = '/root/ChatRWKV/model-latest/RWKV-4-Raven-7B-v6-EngChnJpn-20230331-ctx4096'
...
def chat(item:Item_chat):
    global out1
    n = f'{item.uid}_dummy_server'
    # 加载历史state，如不存在则创建一个
    if n not in all_state:
        save_all_stat('dummy_server', item.uid, out1)
    else:
        out2 = load_all_stat('dummy_server', item.uid)
        save_all_stat('dummy_server', item.uid, out2)
        
    # run
    msg = on_message(item.msg,item.uid)
    if len(msg.strip()) >0:
        return msg
    else:
        return 'ok'

uvicorn.run(app, host="0.0.0.0", port=11142)