...
export const Config: Schema<Config> = Schema.object({
  ...
  background_img: Schema.string().default('https://i0.hdslb.com/bfs/article/b5f0ecfca9b388fe0bc653abf03a36c00498bb73.jpg@942w_1286h_progressive.jpg').description('背景图片url,http开头:'),
...
export function apply(ctx: Context, config: Config) {
  ...
  ctx.command('扫雷活跃榜').alias('ms-ac').action(async () => {
    const json_res: any = await ctx.http.get('http://116.205.167.54:5140/get')
    ...
    today_rank_ms.forEach((i, id) => {
      item1.push(<div style = {`font-size:100px;position: absolute;left:${i[1]}px;top:${i[2]}px;width:${i[3]}px;heigth:${i[4]}px`}>{i[0]}</div>)
    })
    const item2: any[] = [<div>{json_res.update_time}</div>, <div>15p日榜</div>, <br></br>]
    today_rank_pz.forEach((i, id) => {
      item2.push(<div style = {`font-size:100px;position: absolute;left:${i[1]+1080}px;top:${i[2]}px;width:${i[3]}px;heigth:${i[4]}px`}>{i[0]}</div>)
    })
    return <html>
    <img src={bgd_img}style = 'width:2500px;height:4300px'/>
