...
export class McpHub {
  ...
  "mcpServers": {
	...
	private async connectToServer(
      ...
      this.logger.info(`[MCP] Connecting to ${name}...`)
			// Connect
			await client.connect(transport)
      this.logger.info(`[MCP] Connected to ${name}`)
	...
	async restartConnection(serverName: string): Promise<void> {
		...
		if (config) {
			...
			try {
				...
				this.logger.error(`Failed to connect to ${serverName} MCP server`)
			}
		}
		this.isConnecting = false
	}
