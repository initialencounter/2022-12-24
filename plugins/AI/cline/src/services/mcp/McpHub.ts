...
import {
  ...
  McpMode, McpResource,
  McpResourceResponse, McpResourceTemplate,
  McpServer, McpTool, McpToolCallResponse,
...
export class McpHub {
  ...
  logger = new Logger("McpHub")
  constructor(private ctx: Context, private mcpSettingsFilePath: string) {
    this.initializeMcpServers()
  }

  getServers(): McpServer[] {
    // Only return enabled servers
    let servers = this.connections.filter((conn) => !conn.server.disabled).map((conn) => conn.server)
    return servers
  }
  ...
  async getMcpSettingsFilePath(): Promise<string> {
    const fileExists = fssync.existsSync(this.mcpSettingsFilePath)
    if (!fileExists) {
      await fs.writeFile(
  ...
  private async readAndValidateMcpSettingsFile(): Promise<z.infer<typeof McpSettingsSchema> | undefined> {
    try {
      ...
      if (!result.success) {
        this.logger.error("Invalid MCP settings schema.")
        return undefined
      }

      return result.data
    } catch (error) {
      this.logger.error("Failed to read MCP settings:", error)
      return undefined
    }
  }
        ...
        if (stderrStream) {
          stderrStream.on("data", async (data: Buffer) => {
            ...
            if (isInfoLog) {
              ...
              // Treat as error log
              this.logger.error(`Server "${name}" stderr:`, output)
              const connection = this.connections.find((conn) => conn.server.name === name)
              if (connection) {
  ...
  private appendErrorMessage(connection: McpConnection, error: string) {
    const newError = connection.server.error ? `${connection.server.error}/n${error}` : error
  ...
  private setupFileWatcher(name: string, config: any) {
    ...
    if (filePath) {
      ...
      watcher.on("change", () => {
        this.logger.info(`Detected change in ${filePath}. Restarting server ${name}...`)
  ...
  async restartConnection(serverName: string): Promise<void> {
    ...
    if (config) {
      ...
      try {
        ...
        this.logger.info(`${serverName} MCP server connected`)
      } catch (error) {
        this.logger.error(`Failed to restart connection for ${serverName}:`, error)
      }
    }
    this.isConnecting = false
  }
  ...
  public async toggleServerDisabled(serverName: string, disabled: boolean): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        this.logger.error("Settings file not accessible:", error)
      ...
      if (config.mcpServers[serverName]) {
        ...
        if (connection) {
          try {
            ...
            this.logger.error(`Failed to refresh capabilities for ${serverName}:`, error)
          }
        }
      }
    } catch (error) {
      this.logger.error("Failed to update server disabled state:", error)
  ...
  async toggleToolAutoApprove(serverName: string, toolName: string, shouldAllow: boolean): Promise<void> {
    try {
      ...
      this.logger.error("Failed to update autoApprove settings:", error)
      this.logger.error("Failed to update autoApprove settings")
  ...
  public async deleteServer(serverName: string) {
    try {
      ...
      if (config.mcpServers[serverName]) {
        ...
        this.logger.info(`Deleted ${serverName} MCP server`)
      } else {
        this.logger.warn(`${serverName} not found in MCP configuration`)
      }
    } catch (error) {
      this.logger.error(
  ...
  public async updateServerTimeout(serverName: string, timeout: number): Promise<void> {
    try {
      ...
      this.logger.error("Failed to update server timeout:", error)
      if (error instanceof Error) {
        this.logger.error("Error details:", error.message, error.stack)
      }
      this.logger.error(
  ...
  async dispose(): Promise<void> {
    ...
    for (const connection of this.connections) {
      try {
        ...
        this.logger.error(`Failed to close connection for ${connection.server.name}:`, error)
      }
    }
    this.connections = []
