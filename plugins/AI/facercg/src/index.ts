...
import { FaceResponse } from "./type";
import { render } from "./render";
export const name = "facercg";

export const inject = {
  required: ["canvas", "http"],
  optional: ["notifier"],
};
const headers = {
  headers: {
...
export async function get_access_token(ctx: Context, config: Config) {
  ...
  const access_token: string = await (
    await ctx.http.axios(
      `${token_url}&client_id=${config.key}&&client_secret=${config.secret_key}`,
      headers,
    )
  ).data["access_token"];
  return access_token;
...
export async function apply(ctx: Context, config: Config) {
  ...
  ctx
    ...
    .action(async ({ session }) => {
      ...
      let img_url = getImgUrl(session.elements, session?.quote?.elements ?? []);
      if (!img_url) {
      ...
      try {
        buffer = await ctx.http.get(img_url, {
      ...
      //@ts-ignore
      if (ctx?.canvas) {
        const resImg = await render(ctx, buffer, face_arr);
        return h.image(resImg, "image/png");
      }
      return;
    });
}
